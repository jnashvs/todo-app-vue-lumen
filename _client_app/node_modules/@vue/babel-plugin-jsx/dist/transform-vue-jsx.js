...
const transformJSXElement = (path, state) => {
    ...
    if (children.length > 1 || slots) {
        ...
        if (t.isIdentifier(child)) {
            VNodeChild = t.conditionalExpression(t.callExpression(state.get('@vue/babel-plugin-jsx/runtimeIsSlot')(), [child]), child, t.objectExpression([
            ...
            VNodeChild = t.conditionalExpression(t.callExpression(state.get('@vue/babel-plugin-jsx/runtimeIsSlot')(), [t.assignmentExpression('=', slotId, child)]), slotId, t.objectExpression([
...
exports.default = ({
    JSXElement: {
        exit(path, state) {
            path.replaceWith(t.inherits(transformJSXElement(path, state), path.node));
        },
    },
});
